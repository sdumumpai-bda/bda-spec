...
```
/bda-implement docs/80-ImplementPlan/YYYY-MM-DD-HHmm-<slug>.md
...
ถ้า `docs/70-Reference/DesignSystem/` มีอยู่ และ subagent คือ `frontend` หรือ `mobile`:
...
เก็บใน `docs/90-TestPlan/evidence/<plan-slug>/`
...
2. Append section ใน plan:
   ...
   - Files changed: <list>
   - Tests added: <list>
   - Evidence: docs/90-TestPlan/evidence/<slug>/
   - Subagent used: <name>
   - Time: <duration>
   ```
3. Update `docs/00-Index/IMPLEMENTATION-STATUS.md` mark feature/phase done
